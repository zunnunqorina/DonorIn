...
if (isset($_GET['hapus']) && is_numeric($_GET['hapus'])) {
    $stmt = $conn->prepare("DELETE FROM event_donor WHERE id = ?");
    $stmt->execute([(int) $_GET['hapus']]);
    header("Location: event_donor.php?pesan=hapus_sukses");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['aksi'] ?? '') === 'tambah') {
    ...
    if (empty($judul) || empty($lokasi) || empty($kota) || empty($tanggal) || empty($jam_mulai) || empty($jam_selesai)) {
        ...
        $stmt = $conn->prepare("INSERT INTO event_donor
            (judul, deskripsi, lokasi, alamat, kota, tanggal, jam_mulai, jam_selesai, kuota, penyelenggara, kontak, status)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
        if ($stmt->execute([$judul, $deskripsi, $lokasi, $alamat, $kota, $tanggal, $jam_mulai, $jam_selesai, $kuota, $penyelenggara, $kontak, $status])) {
            header("Location: event_donor.php?pesan=tambah_sukses");
            exit();
        } else {
            $error_tambah = 'Gagal menambahkan event!';
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['aksi'] ?? '') === 'edit') {
    ...
    if (empty($judul) || empty($lokasi) || empty($kota) || empty($tanggal) || empty($jam_mulai) || empty($jam_selesai)) {
        ...
        $stmt = $conn->prepare("UPDATE event_donor SET
            judul=?, deskripsi=?, lokasi=?, alamat=?, kota=?, tanggal=?,
            jam_mulai=?, jam_selesai=?, kuota=?, penyelenggara=?, kontak=?, status=?
            WHERE id=?");
        if ($stmt->execute([$judul, $deskripsi, $lokasi, $alamat, $kota, $tanggal, $jam_mulai, $jam_selesai, $kuota, $penyelenggara, $kontak, $status, $id_edit])) {
            header("Location: event_donor.php?pesan=edit_sukses");
            exit();
        } else {
...
$params = [];
if ($search !== '') {
    $where   .= " AND (judul LIKE ? OR lokasi LIKE ? OR kota LIKE ? OR penyelenggara LIKE ?)";
    $s        = "%$search%";
    $params   = [$s, $s, $s, $s];
}
if ($filter_status !== '') {
...
$q_total = $conn->prepare("SELECT COUNT(*) FROM event_donor $where");
...
// Statistik
$stat_total  = (int) $conn->query("SELECT COUNT(*) FROM event_donor")->fetchColumn();
$stat_aktif  = (int) $conn->query("SELECT COUNT(*) FROM event_donor WHERE status='aktif'")->fetchColumn();
$stat_selesai= (int) $conn->query("SELECT COUNT(*) FROM event_donor WHERE status='selesai'")->fetchColumn();
$stat_bulan  = (int) $conn->query("SELECT COUNT(*) FROM event_donor WHERE MONTH(tanggal)=MONTH(CURDATE()) AND YEAR(tanggal)=YEAR(CURDATE())")->fetchColumn();
...
$total_ev_sosial = $conn->query("SELECT COUNT(*) FROM event_sosialisasi WHERE status='aktif'")->fetchColumn() ?? 0;
...
<head>
    ...
    <title>Event Donor Darah — DonorIn Admin</title>
    ...
    <style>
        ...
        .event-tanggal-box {
            ...
            background:var(--merah); border-radius:10px; color:#fff;
        }
        ...
        .td-event-info .meta i { color:var(--merah); margin-right:2px; }
        .jam-badge { display:inline-flex; align-items:center; gap:5px; padding:3px 10px; background:var(--merah-tipis); border:1px solid var(--merah-muda); border-radius:20px; font-size:11px; font-weight:700; color:var(--merah); }
        .kuota-badge { display:inline-flex; align-items:center; gap:4px; padding:3px 10px; background:#EDF4FF; border:1px solid #BFDBFE; border-radius:20px; font-size:11px; font-weight:700; color:#1565C0; }
    </style>
</head>
<body>
...
<main class="main">
    ...
    <div class="content">
        ...
        <div class="notif notif-sukses"><i class="fas fa-check-circle"></i> Event berhasil ditambahkan!</div>
        <?php elseif ($pesan === 'edit_sukses'): ?>
        <div class="notif notif-sukses"><i class="fas fa-check-circle"></i> Data event berhasil diperbarui!</div>
        <?php elseif ($pesan === 'hapus_sukses'): ?>
        <div class="notif notif-sukses"><i class="fas fa-check-circle"></i> Event berhasil dihapus.</div>
        <?php endif; ?>
        <?php if ($error_tambah): ?>
        ...
        <div class="mini-stats" style="grid-template-columns:repeat(4,1fr);">
            <div class="mini-card">
                <div class="mini-icon" style="background:#EDF4FF;color:#2563EB;"><i class="fas fa-calendar-alt"></i></div>
                <div><div class="mini-val"><?= $stat_total ?></div><div class="mini-label">Total Event</div></div>
            </div>
            <div class="mini-card">
                <div class="mini-icon" style="background:#E8F8F0;color:#1B8A4E;"><i class="fas fa-check-circle"></i></div>
        ...
        <div class="panel">
            <div class="panel-head">
                <div class="panel-title">
                    <i class="fas fa-calendar-alt"></i> Daftar Event Donor Darah
                ...
                <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
                    ...
                    <form method="GET" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
                        ...
                        <a href="event_donor.php" class="btn btn-ghost btn-sm"><i class="fas fa-times"></i> Reset</a>
                        <?php endif; ?>
                    </form>
                ...
                <table class="tbl">
                    ...
                    ?>
                        <tr>
                            ...
                            <td style="font-size:12px;color:var(--teks-sedang);">
                    ...
                    <?php endforeach; else: ?>
                        <tr><td colspan="7">
                            <div class="empty-state">
                                ...
                                <p><?= ($search || $filter_status) ? 'Tidak ada event yang cocok dengan filter.' : 'Belum ada event donor darah. Tambahkan yang pertama!' ?></p>
                            </div>
                        </td></tr>
                    <?php endif; ?>
                    </tbody>
...
<div class="modal-overlay" id="modalTambah">
    <div class="modal" style="max-width:680px;">
        <div class="modal-head">
            <div class="modal-title"><i class="fas fa-calendar-plus"></i> Tambah Event Donor Darah</div>
            <button class="modal-close" onclick="tutupModal('modalTambah')"><i class="fas fa-times"></i></button>
        </div>
        <form method="POST" action="event_donor.php">
            <input type="hidden" name="aksi" value="tambah">
            <div class="modal-body">
                <div class="form-grid">
                    <div class="form-group full">
                        <label class="form-label">Judul Event <span class="req">*</span></label>
                        <input type="text" name="judul" class="form-input" placeholder="Contoh: Donor Darah HUT RI ke-80" required>
                    </div>
                    <div class="form-group full">
                        <label class="form-label">Deskripsi</label>
                        <textarea name="deskripsi" class="form-input" rows="2" placeholder="Deskripsi singkat kegiatan..."></textarea>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Lokasi / Tempat <span class="req">*</span></label>
                        <input type="text" name="lokasi" class="form-input" placeholder="Contoh: Gedung Rektorat" required>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="kuota" class="form-input" placeholder="0" min="0">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="penyelenggara" class="form-input" placeholder="Contoh: PMI Kota Mataram">
                    </div>
                    <div class="form-group">
...
<div class="modal-overlay" id="modalEdit">
    <div class="modal" style="max-width:680px;">
        <div class="modal-head">
            <div class="modal-title"><i class="fas fa-pen"></i> Edit Event Donor Darah</div>
        ...
        <form method="POST" action="event_donor.php">
            ...
            <div class="modal-body">
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        <label class="form-label">Kuota Peserta</label>
                        <input type="number" name="kuota" id="edit_kuota" class="form-input" min="0">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Jam Mulai <span class="req">*</span></label>
                    ...
                    <div class="form-group">
                        <label class="form-label">Penyelenggara</label>
                        <input type="text" name="penyelenggara" id="edit_penyelenggara" class="form-input">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Kontak</label>
            ...
            <div class="modal-foot">
                ...
                <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Perbarui Event</button>
            </div>
        </form>
    </div>
</div>
...
<div class="modal-overlay" id="modalHapus">
    <div class="modal" style="max-width:400px;">
        <div class="modal-body" style="padding:32px 28px;text-align:center;">
            ...
            <h3 style="font-size:18px;font-weight:800;margin-bottom:8px;">Hapus Event?</h3>
