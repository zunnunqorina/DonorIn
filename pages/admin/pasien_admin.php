...
$q_pasien = $conn->prepare("SELECT * FROM user $where ORDER BY tanggal_daftar DESC LIMIT ? OFFSET ?");
foreach ($params as $i => $val) {
    $q_pasien->bindValue($i + 1, $val);
}
$param_count = count($params);
$q_pasien->bindValue($param_count + 1, $per_page, PDO::PARAM_INT);
$q_pasien->bindValue($param_count + 2, $offset,   PDO::PARAM_INT);
$q_pasien->execute();
        ...
        <form method="POST" action="pasien_admin.php">
            ...
            <div class="modal-body">
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        <label class="form-label">Golongan Darah Dibutuhkan <span class="req">*</span></label>
                        <select name="goldar_dibutuhkan" class="form-select" required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="nama_rs" class="form-input" placeholder="Contoh: RSUD Mataram">
                    </div>
                    <div class="form-group full">
        ...
        <form method="POST" action="pasien_admin.php">
            ...
            <div class="modal-body">
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        <label class="form-label">Golongan Darah Dibutuhkan <span class="req">*</span></label>
                        <select name="goldar_dibutuhkan" id="edit_goldar" class="form-select" required>
                            <option value="">-- Pilih --</option>
                            <?php foreach (['A','B','O','AB'] as $g): ?>
                            <option value="<?= $g ?>"><?= $g ?></option>
                            <?php endforeach; ?>
                    ...
                    <div class="form-group">
                        <label class="form-label">Nama Rumah Sakit</label>
                        <input type="text" name="nama_rs" id="edit_nama_rs" class="form-input" placeholder="Contoh: RSUD Mataram">
                    </div>
                    <div class="form-group full">
                        <label class="form-label">Alamat Lengkap</label>
                        <textarea name="alamat" id="edit_alamat" class="form-input" rows="2"
                            style="resize:vertical;"></textarea>
                    </div>
                </div>
            </div>
...
function bukaModalEdit(data) {
    ...
    document.getElementById('edit_goldar').value   = data.goldar_dibutuhkan || '';
    document.getElementById('edit_nama_rs').value  = data.nama_rs    || '';
    document.getElementById('edit_alamat').value   = data.alamat     || '';
    bukaModal('modalEdit');
}

function konfirmasiHapus(id, nama) {
    hapusDataSweet(id, nama, 'pasien_admin.php?hapus=');
}
